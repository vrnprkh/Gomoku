...
if __name__ == "__main__":
    ...
    # users
    userInfoPath = os.path.join(dirname, "prodData/users.txt")
    cursor.execute("DELETE FROM `Users`;")
    query = load_data_into_table("Users", userInfoPath, queryPath)
    cursor.execute(query)

    # games
    gameDataPath = os.path.join(dirname, "prodData/games.txt")
    cursor.execute("DELETE FROM `Games`;")
    query = load_data_into_table("Games", gameDataPath, queryPath)
    cursor.execute(query)

    # friends
    friendDataPath = os.path.join(dirname, "prodData/friends.txt")
    ...
    #favourites
    favouritesPath = os.path.join(dirname, "prodData/favourites.txt")
    cursor.execute("DELETE FROM `FavouriteGames`;")
    query = load_data_into_table("FavouriteGames", favouritesPath, queryPath)
    cursor.execute(query)

    #detailed moves
    detailedMovesPath = os.path.join(dirname, "prodData/detailedMoves.txt")
